<?php
/**
 * Homepage flow band: the who-is-in-the-room diagram.
 *
 * HARDCODED ON PURPOSE, same reasoning as the globe, plus one of its own:
 * flow.js registers clipPath ids (cx-flow-limb) and resolves node positions
 * with getScreenCTM at runtime. Two instances of this section on one page
 * collide on those ids and the diagram breaks in a way that looks like a
 * rendering glitch rather than a duplication. A flexible-content layout would
 * make duplicating it a one-click action, so it is not one.
 *
 * @package convergx
 */
...
?>
<section class="flow-band" aria-labelledby="home-flow-title">
    <div class="wrap">
      <h2 id="home-flow-title" class="vh">How a requirement reaches a company that can answer it</h2>
      ...
      <figure class="flow">
        ...
        <svg class="flow-art" viewBox="0 0 1600 540" preserveAspectRatio="none" aria-hidden="true" focusable="false">
          <!-- FEED. A requirement leaving an industry for ConvergX. EIGHT
               ...
               real and overwrites all eight, because the end of a word moves
          ...
          <!-- RESTORED 2026-08-12: Chip kept the industry column as a VISUAL after the
           Industries tab came out of the nav. Names only, no links: the pages are
           staged unlinked for a later release. -->
