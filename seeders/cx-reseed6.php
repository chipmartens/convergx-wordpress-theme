 ...
 */
$pages = json_decode( file_get_contents('/tmp/cx-full7.json'), true );
...
foreach ( $pages as $path => $secs ) {
    ...
    foreach ( $secs as $s ) {
        ...
        $rows[] = array(
            ...
            'body' => $s['body'], 'say' => ($s['say'] ?? ''), 'level' => ($s['level'] ?? 2), 'links' => ($s['links'] ?? array()), 'twopath' => ($s['twopath'] ?? array()), 'dense' => (int)$s['dense'], 'surface' => $sf,
        );
    }
